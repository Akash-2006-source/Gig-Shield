...
const Claim = sequelize.define('Claim', {
  ...
  // ── Decision-engine verdict (Phase 1 unified payout gate) ────────────────
  // Populated by decisionEngine.gateClaimForPayout at payout time.
  //   AUTO   — all checks passed, disbursement proceeded
  //   REVIEW — fraud score in soft band; held for admin
  //   BLOCK  — hard-failed policy/fraud/reserve check
  decision_verdict: {
    type:      DataTypes.ENUM('AUTO', 'REVIEW', 'BLOCK'),
    allowNull: true
  },
  // Array of human-readable reasons per check (policy/fraud/reserve).
  // Shape: { policy: [...], fraud: [...], reserve: [...], fraudScore? }
  decision_reasons: {
    type:         DataTypes.JSON,
    allowNull:    true,
    defaultValue: null
  ...
  // ── Ground-truth label (Phase 5 ML training input) ──────────────────────
  // Populated by admin after post-hoc review. Starts 'unlabeled'; flipping
  // to fraud_confirmed/false_positive/clean builds the training set.
  label: {
    type:         DataTypes.ENUM('unlabeled', 'clean', 'fraud_confirmed', 'false_positive'),
    allowNull:    false,
