...
// ── Behavior ingest rate limiter ─────────────────────────────────────────────
// 60 writes per minute per IP. Accommodates the client's 60s active-sampling
// rhythm with headroom; anything above this is a buggy/malicious client.
const behaviorLimiter = rateLimit({
  windowMs:  60 * 1000,
  max:       60,
  message:   { message: 'Behavior event ingest rate limit exceeded.' },
  standardHeaders: true,
  legacyHeaders:   false,
})
