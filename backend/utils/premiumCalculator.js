 ...
 * @param {number}  [params.behaviorMultiplier=1.0] - Premium multiplier from
 *                                              behaviorModifier.modifiersFor;
 *                                              expected range [0.90, 1.20].
 *                                              Pass 1.0 (or omit) to disable.
 * @returns {object}  { premium, basePremium, behaviorMultiplier, weeklyEarnings, contributionRate, contributionPct, coverage }
 */
const calculateContributionPremium = ({ planType = 'Standard', avgDailyEarnings = 700, behaviorMultiplier = 1.0 } = {}) => {
  ...
  const basePremium = Math.round(rawPrem / 5) * 5  // round to nearest ₹5

  // Apply behavior multiplier, then re-round to nearest ₹5 so the final
  // price remains cash-register-friendly.
  const mult      = Number.isFinite(behaviorMultiplier) && behaviorMultiplier > 0 ? behaviorMultiplier : 1.0
  const adjusted  = basePremium * mult
  const premium   = Math.round(adjusted / 5) * 5
 ...
 * @returns {object}
 */
const calculateWeeklyPremium = ({ city = 'default', planType = 'Standard', avgDailyEarnings = 700, behaviorMultiplier = 1.0 } = {}) => {
  const daily = Math.min(parseFloat(avgDailyEarnings) || 700, 5000)

  // ── Layer 2: worker-facing contribution price ──────────────────────────────
  const contribution = calculateContributionPremium({ planType, avgDailyEarnings: daily, behaviorMultiplier })
 ...
 *
 * The `behaviorMultiplier` is applied as a SOFT modifier (expected range
 * [0.85, 1.10] from behaviorModifier). A `guaranteedFloorFraction` hard-caps
 * how far the multiplier can reduce the payout — even a multiplier of 0.0
 * can't drop below `floorFraction × cappedPayout`. This is the legal/fairness
 * safety net: the behavior model nudges, never guts, a worker's claim.
 *
 * @param {object} opts
 * @param {number} opts.avgDailyEarnings
 * @param {number} opts.workHoursPerDay
 * @param {number} opts.hoursLost
 * @param {number} opts.coverageCap
 * @param {number} [opts.behaviorMultiplier=1.0]        Soft modifier in [0.85, 1.10]
 * @param {number} [opts.guaranteedFloorFraction=0.85]  Absolute floor as share of raw
 * @returns {number} payout in ₹, rounded to nearest 50
 */
const calculatePayout = ({
  ...
  const mult     = Number.isFinite(behaviorMultiplier) && behaviorMultiplier > 0 ? behaviorMultiplier : 1.0
  const adjusted = cappedPayout * mult

  // Guaranteed minimum: even with a low behavior score, payout can't drop
  // below this fraction of the un-adjusted capped amount. Never exceeds
  // the upper-bound raise from the multiplier (that's up to mult itself).
  const floor    = cappedPayout * guaranteedFloorFraction
  const final    = Math.max(adjusted, floor)
...
const PLAN_CONFIG = {
  basic: {
    ...
    // Per-user payout ceilings — decision engine enforces.
    // daily = 50% of coverage, weekly = 100% of coverage. Guards against
    // rapid-fire claims draining the entire cap in one bad-weather burst.
    dailyPayoutCap:          1250,
