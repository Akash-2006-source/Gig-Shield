...
require('./models/BehaviorEvent')
require('./models/UserScore')
require('./models/ClaimLabel')
...
if (DISABLE_JOBS) {
  ...
  // Behavior score rollup — fires at 3:00 AM local, then every 24h.
  // Reads last 7 days of BehaviorEvents, appends one UserScore row per active user.
  const computeBehaviorScores = require('./jobs/computeBehaviorScores')
  const runBehaviorScoring    = () => jobScheduler.runJob('compute_behavior_scores', computeBehaviorScores)
  const firstBehaviorAt       = msUntilNextLocalHour(3)
  setTimeout(() => {
    runBehaviorScoring()
    setInterval(runBehaviorScoring, ONE_DAY_MS)
  }, firstBehaviorAt)
  ...
  const firstBehaviorAt_readable   = new Date(Date.now() + firstBehaviorAt).toLocaleString()
  ...
  console.log(`[jobs] Behavior score rollup:      first run at ${firstBehaviorAt_readable}, then every 24h`)
}
