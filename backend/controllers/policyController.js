...
const { getLatestScore } = require('../services/userScoreService')
const { modifiersFor }   = require('../utils/behaviorModifier')
...
exports.createPolicy = async (req, res) => {
  try {
    ...
    // Behavior score → premium multiplier. New users neutralise to 1.0× so
    // they're not rewarded OR punished before we have data on them.
    const scoreRow           = await getLatestScore(req.user.id)
    const { premiumMultiplier } = modifiersFor(scoreRow)
    const premiumResult      = calculateWeeklyPremium({ city, planType: type, avgDailyEarnings, behaviorMultiplier: premiumMultiplier })
