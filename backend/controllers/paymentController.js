...
const { gateClaimForPayout } = require('../services/decisionEngine')
...
exports.disbursePayout = async (req, res) => {
  try {
    ...
    // ── Unified decision gate (Phase 1) ───────────────────────────────────
    // Single chokepoint: Policy → Fraud → Reserve. No other checks allowed
    // here — if the gate says AUTO we disburse, otherwise we park or reject.
    const verdict = await gateClaimForPayout(claimId, payoutAmount)

    // Persist verdict on the claim for audit, regardless of outcome.
    const claim = await Claim.findByPk(claimId)
    if (claim) {
      await claim.update({
        decision_verdict:     verdict.action,
        decision_reasons:     verdict,
        decision_checked_at:  new Date()
      })
    }

    if (verdict.action === 'BLOCK') {
      const isReserveBlock = verdict.checks?.reserve?.code === 'INSUFFICIENT_RESERVES'
      if (isReserveBlock && claim) {
        await claim.update({
          status:          'halted_insufficient_reserves',
          payout_status:   'failed',
          payoutUpdatedAt: new Date(),
          notes:           `${claim.notes || ''}\n[${new Date().toISOString()}] Payout blocked: ${verdict.reasons.join('; ')}`.trim()
        })
        ...
        return res.status(503).json({
          ...
          currentRatio: verdict.checks.reserve.currentRatio,
          claimStatus:  'halted_insufficient_reserves'
        })
      }
      console.warn(`[paymentController] Payout BLOCKED claimId=${claimId} reasons=${verdict.reasons.join(' | ')}`)
      return res.status(403).json({
        message:     'Payout rejected by decision engine.',
        code:        'PAYOUT_BLOCKED',
    ...
    if (verdict.action === 'REVIEW') {
      if (claim) {
        await claim.update({
          status: 'flagged',
          notes:  `${claim.notes || ''}\n[${new Date().toISOString()}] Held for review: ${verdict.reasons.join('; ')}`.trim()
        })
      }
      console.warn(`[paymentController] Payout REVIEW claimId=${claimId} reasons=${verdict.reasons.join(' | ')}`)
      return res.status(202).json({
        message:     'Payout held for admin review.',
        code:        'PAYOUT_UNDER_REVIEW',
        reasons:     verdict.reasons,
        claimStatus: 'flagged'
      })
    }

    // action === 'AUTO' — proceed. Allocate liquidity BEFORE disbursing.
