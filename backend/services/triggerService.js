...
const { getLatestScore } = require('./userScoreService')
  ...
  const currentHour = new Date().getHours()
    ...
    for (const policy of policies) {
      ...
      for (const trigger of firedTriggers) {
        ...
        // Behavior score → soft payout multiplier. Bounded narrow (0.85–1.10)
        // and floored at 0.85 × raw by calculatePayout so the model can never
        // gut a legitimate claim — it only nudges. Neutral 1.0 for new users.
        const scoreRow = await getLatestScore(user.id)
        const { payoutMultiplier } = modifiersFor(scoreRow)
        ...
        const payoutAmount = calculatePayout({
          ...
          coverageCap,
          behaviorMultiplier: payoutMultiplier
        })
