...
class ReserveService {
  ...
  /**
   * Public inflow helper — credits a positive amount to a reserve pool.
   * This is the base ledger-entry primitive for premiums, reinsurance top-ups,
   * and manual admin deposits. Idempotent via UNIQUE(reserve_type, reference)
   * with a SELECT-first guard (re-runs return the existing row, never throw).
   *
   * @param {string} reserveType   liquidity | reinsurance | operational | claims_pending
   * @param {number} amount        positive number — deposits only (use createReserve for deductions)
   * @param {string} reference     REQUIRED idempotency key (e.g. `premium:{charge_id}`)
   * @param {object} [metadata={}] free-form context for audit/debug
   * @param {object} [transaction] sequelize transaction — must be passed when credit is part of a larger atomic write
   */
  async credit(reserveType, amount, reference, metadata = {}, transaction = undefined) {
    if (!reference)                          throw new Error('reference is required for credit() — idempotency key missing')
    if (!Number.isFinite(amount) || amount <= 0) throw new Error('credit() amount must be a positive finite number')

    return this.createReserve(reserveType, amount, {
      reference,
      metadata,
      transaction
    })
