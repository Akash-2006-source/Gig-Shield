...
class PremiumCollectionService {
  ...
  async collectDailyPremium(userId, date = new Date()) {
    ...
    if (platformResult.success) {
      await this._commitSuccessfulCharge(charge, policy, today, {
        payment_method: 'platform_earnings',
  ...
  /**
   * Atomic commit of a successful premium charge:
   *   1. flip PremiumCharge status → 'success'
   *   2. reset policy's collection-health counters
   *   3. credit liquidity reserve (+amount, reference = `premium:{charge.id}`)
   *
   * All three run inside one sequelize.transaction so a mid-flight failure
   * rolls back every write — this is the financial consistency guarantee.
   * Idempotency comes from reserveService.credit (UNIQUE reserve_type+reference)
   * plus the existing PremiumCharge UNIQUE(user, policy, charge_date).
   */
  async _commitSuccessfulCharge(charge, policy, todayStr, chargeFields, userId, amount) {
    await sequelize.transaction(async (t) => {
      ...
      await reserveService.credit(
        ...
        `premium:${charge.id}`,
        {
          ...
          method:    chargeFields.payment_method
        },
        t
      )
    })
