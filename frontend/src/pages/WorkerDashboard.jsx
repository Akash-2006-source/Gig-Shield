...
import { useBehaviorSampler } from '../hooks/useBehaviorSampler'
...
const WorkerDashboard = () => {
  ...
  const [payoutDraft, setPayoutDraft] = useState({ payoutMethod: 'UPI', payoutHandle: '', payoutAccountName: '' })
  ...
  // Stream sampled locations to /api/behavior/ingest. Fires every minute
  // the hook emits a new sample. Adaptive cadence + client-side dedup live
  // inside the hook.
  useBehaviorSampler({
    location: locationTracker.location,
    enabled: locationTracker.isTracking
  })
  ...
  // 1. Auto-start tracking on dashboard mount — always. The hook prompts
  // the browser once for permission (or silently resumes if previously
  // granted), then auto-samples every 60 seconds.
  useEffect(() => {
  ...
  useEffect(() => {
    if (locationTracker.location && !syncLock.current) {
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        ...
        {/* Auto-sync status — no manual action required */}
        {locationTracker.error ? (
          <section className="dashboard-section">
            <div className="info-card" style={{ background: '#fee2e2', borderLeft: '4px solid #dc2626', padding: '0.75rem 1rem' }}>
              <span style={{ color: '#991b1b', fontSize: 14 }}>
                📍 Location access blocked — {locationTracker.error}. Enable GPS in your browser to activate auto-sync.
              </span>
            </div>
          </section>
        ) : (
          <section className="dashboard-section">
            <div className="info-card" style={{ background: '#ecfdf5', borderLeft: '4px solid #10b981', padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <span style={{ fontSize: 18 }}>📍</span>
              <span style={{ color: '#065f46', fontSize: 14 }}>
                {locationTracker.location
                  ? <>Auto-syncing location every minute — last fix {new Date(locationTracker.location.timestamp).toLocaleTimeString()}</>
                  : <>Auto-sync active — acquiring first GPS fix…</>
                }
              </span>
            </div>
          </section>
          ...
          <div className="info-card" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <button
                ...
                onClick={() => {
                  setPayoutDraft({
                    payoutMethod: dashboardData?.user?.payoutMethod || 'UPI',
                    payoutHandle: dashboardData?.user?.payoutHandle || '',
                    payoutAccountName: dashboardData?.user?.payoutAccountName || ''
                  })
                  setPayoutError('')
                ...
                style={{ fontSize: 13 }}
              >
                {dashboardData?.user?.payoutHandle ? 'Edit →' : 'Add UPI / Bank →'}
              </button>
            </div>
            ...
            {dashboardData?.user?.payoutHandle ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: '#fff', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: '24px' }}>{dashboardData.user.payoutMethod === 'UPI' ? '📱' : '🏦'}</div>
                <div>
                  <div style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>
                    {dashboardData.user.payoutMethod === 'UPI' ? 'UPI ID' : 'Bank account'}
                  </div>
                  <div style={{ fontWeight: '600', color: '#0f172a', fontFamily: 'monospace', fontSize: '15px' }}>
                    {dashboardData.user.payoutHandle}
                  </div>
                  {dashboardData.user.payoutAccountName && (
                    <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>
                      Account name: {dashboardData.user.payoutAccountName}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ background: '#fef3c7', padding: '1rem', borderRadius: '8px', border: '1px solid #fcd34d' }}>
                <div style={{ fontWeight: 600, color: '#92400e', marginBottom: 4 }}>⚠️ No payout destination set</div>
                <div style={{ fontSize: 13, color: '#92400e' }}>
                  Approved claims can't be disbursed until you add your UPI ID or bank details.
                </div>
              </div>
            )}
          </div>
      ...
      {showPayoutModal && (
        <div
          onClick={() => !payoutSaving && setShowPayoutModal(false)}
          style={{
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0 }}>💳 Payout destination</h3>
              <button
                type="button"
                onClick={() => !payoutSaving && setShowPayoutModal(false)}
            ...
            {payoutError && <div className="error-message" style={{ marginBottom: '1rem' }}>{payoutError}</div>}

            <div className="form-group" style={{ marginBottom: '1rem' }}>
              <label>Method</label>
              <select
                value={payoutDraft.payoutMethod}
                onChange={(e) => setPayoutDraft({ ...payoutDraft, payoutMethod: e.target.value })}
              >
            ...
            <div className="form-group" style={{ marginBottom: '1rem' }}>
              <label>{payoutDraft.payoutMethod === 'UPI' ? 'UPI ID' : 'Account details'}</label>
              <input
                type="text"
                value={payoutDraft.payoutHandle}
                onChange={(e) => setPayoutDraft({ ...payoutDraft, payoutHandle: e.target.value })}
                placeholder={payoutDraft.payoutMethod === 'UPI' ? 'yourname@upi' : 'Account number or IBAN'}
              />
            </div>
            ...
            <div className="form-group" style={{ marginBottom: '1.5rem' }}>
              ...
              <input
                type="text"
                value={payoutDraft.payoutAccountName}
                onChange={(e) => setPayoutDraft({ ...payoutDraft, payoutAccountName: e.target.value })}
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                ...
                onClick={async () => {
                  const handle = payoutDraft.payoutHandle.trim()
                  if (!handle) {
                    setPayoutError(payoutDraft.payoutMethod === 'UPI' ? 'UPI ID is required' : 'Account details are required')
                    return
                  }
                  ...
                  try {
                    await updateProfile({
                      payoutMethod: payoutDraft.payoutMethod,
                      payoutHandle: handle,
                      payoutAccountName: payoutDraft.payoutAccountName.trim() || undefined,
