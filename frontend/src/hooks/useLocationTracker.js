...
export const useLocationTracker = (autoSyncInterval = 60000) => {
  ...
  const startTracking = useCallback(() => {
    ...
    // Kick off the first fetch immediately. If permission is already granted
    // we go straight to syncLocation so the UI doesn't sit "acquiring fix"
    // for 60s waiting on the first interval tick. If permission is NOT yet
    // granted, requestLocationOnce triggers the browser prompt which itself
    // produces a fix on accept.
    if (hasPermissionGranted()) {
      syncLocation()
    } else {
      requestLocationOnce()
